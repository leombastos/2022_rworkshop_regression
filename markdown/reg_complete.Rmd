...
-   If too advanced, stay for the stats concepts, catch up on coding later.
...
# Model fitting
reg_lin_mod <- lmer(yield_mgha ~ (1|rep) + sr_ksha, 
                    data = reg_dfw)
...
randeff_lin_rep <- ranef(reg_lin_mod)[[1]] 
...
nd <- nd %>%
  mutate(yield_pred_lin_mgha = predict(reg_lin_mod, nd, re.form = NA))
...
# Model fitting
reg_quad_mod <- lmer(yield_mgha ~ (1|rep) + sr_ksha + I(sr_ksha^2),
                     data = reg_dfw)
...
```{r LP model}
reg_linp_mod <- nlme(yield_mgha ~ f.lrp(sr_ksha, a, b, t.x), 
                 data = reg_dfw, 
...
# Creating predictions
nd <- nd %>%
  mutate(yield_pred_linp_mgha = predict(reg_linp_mod, nd, level = 0))
...
reg_dfw <- reg_dfw %>%
  ...
  mutate(yield_pred_quad_mgha = predict(reg_quad_mod, 
                                  re.form = NA)) %>%
  # Predicting LP
...
reg_dfw %>%
  pivot_longer(cols=c(yield_pred_quad_mgha, yield_pred_linp_mgha),
              names_to = "model",
              values_to = "yield_pred"
               ) %>%
  group_by(model) %>%
  summarise(rmse = metrica::RMSE(obs =  yield_mgha, pred = yield_pred)) %>%
  unnest(rmse) %>%
  bind_cols(data.frame(AIC=c(AIC(reg_linp_mod), 
                             AIC(reg_quad_mod))))
...
LP has a slightly higher RMSE (not desired) and lowest AIC (desired).
...
```{r linp optimum}
reg_linp_mod$coefficients$fixed
...
yosr <- data.frame(sr_ksha = 73.4770944) 


yosr <- yosr %>%
  mutate(yosr_mgha = predict(reg_linp_mod, yosr, level=0))


ggplot(reg_dfw, aes(x = sr_ksha, y = yield_mgha))+
  geom_point(size = 3, alpha = .7)+
  geom_vline(aes(xintercept = 73.4770944), linetype = 2)+
  geom_hline(data = yosr, aes(yintercept = yosr_mgha), linetype = 2)+
  geom_line(data = nd, aes(y = yield_pred_linp_mgha), 
            color = "blue",
            size = 1.5)
